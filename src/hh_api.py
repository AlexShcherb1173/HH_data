...
class HHApi:
    ...
    def __init__(self, area: int = 113, per_page: int = 50,
                 connect_timeout: int = 5, read_timeout: int = 10):
        """
        ...
        self.session.headers.update({              # Устанавливаем кастомный заголовок User-Agent.
    ...
    def get_companies(self, text: str = "IT") -> List[Dict]:
        ...
        while len(companies) < 15:
            ...
            params = {"text": text, "area": self.area, "page": page, "per_page": 50}  # Запрашиваются данные с
                                                                                      # параметрами:
            ...
            if page >= data.get("pages", 1) - 1:
                break
            page += 1
        return companies[:15]  #  Возвращается список максимум из 15 компаний.
    ...
    def get_vacancies_for_company(self, employer_id: int) -> List[Dict]:
        ...
        while True:  # В бесконечном цикле (while True):
            ...
            for v in data.get("items", []): # Для каждой вакансии в data["items"]:
                ...
                vacancies.append({     # Формируется словарь с ключевыми данными и добавляется в список vacancies.:
